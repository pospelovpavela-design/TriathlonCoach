        ...
        NavigationStack {
                    ...
                    VStack(spacing: 16) {
                        ...
                        if onUpdate != nil {
                            Button(action: { showLog = true }) {
                                HStack {
                                    Image(systemName: workout.completed ? "checkmark.circle.fill" : "pencil.circle")
                                    Text(workout.completed ? "Результат записан" : "Записать результат")
                                        .fontWeight(.semibold)
                                }
                                .font(.system(size: 15))
            ...
            .sheet(isPresented: $showLog) {
                if let update = onUpdate {
                    LogWorkoutSheet(workout: workout, onSave: { updated in
                        update(updated)
                        dismiss()
                    })
                }
